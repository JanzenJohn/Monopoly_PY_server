                ...
                if structure[0] == "done":
                    ...
                    for i in user.debt:
                        if user.money >= int(i[1]):
                            ...
                            input(f"You dont have enough money ({i[1]} to pay {i[0]})")
                    ...
                    if not sc.board[user.position].owned:
                        ...
                        input(f"{sc.board[user.position].name} isn't available for purchase")
                elif structure[0] == "mortgage":
                    try:
                        int(structure[1])
                    except ValueError or IndexError:
                        input("ID not Present or invalid")
                        continue
                    id = int(structure[1])
                    if sc.board[id].houses > 0:
                        input(f"{sc.board[id].name} has houses on it.")
                        continue
                    if sc.board[id].owner_id != user.id:
                        input(f"You dont own {sc.board[id].name}")
                        continue
                    if sc.board[id].morg:
                        input(f"{sc.board[id].name} already has a mortgage")
                        continue
                    if sc.board[id].houses > 0:
                        input(f"{sc.board[id].name} has houses sell them first")
                        continue
                    ...
                    if not sc.board[id].morg:
                        input(f"{sc.board[id].name} doensn't have a morgage")
                        continue
                    if int(int(sc.board[id].price) / 2) > user.money:
                        input(f"You dont have enough money to clear {sc.board[id].name}")
                        continue
